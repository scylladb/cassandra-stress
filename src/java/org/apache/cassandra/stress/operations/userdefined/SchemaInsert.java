...
public class SchemaInsert extends SchemaStatement
{
    ...
    private class JavaDriverV4Run extends Runner
    {
        ...
        public boolean run() throws Exception
        {
            List<shaded.com.datastax.oss.driver.api.core.cql.BoundStatement> stmts = new ArrayList<>();
            partitionCount = partitions.size();
            ...
            for (int j = 0; j < stmts.size(); j += 65535)
            {
                List<shaded.com.datastax.oss.driver.api.core.cql.BoundStatement> substmts = stmts.subList(j, Math.min(j + stmts.size(), j + 65535));
                ...
                else
                {
                    ...
                    batch.addStatements((BatchableStatement<?>) substmts);
                    stmt = batch.build();
                }
