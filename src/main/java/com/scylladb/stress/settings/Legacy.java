...
public class Legacy implements Serializable {
    ...
    static {
        ...
        availableOptions.addOption("tf", "transport-factory", true, "Fully-qualified TTransportFactory class name for creating a connection. Note: For Thrift over SSL, use com.scylladb.stress.SSLTransportFactory.");
    ...
    public static StressSettings build(String[] arguments) {
        ...
        try {
            ...
            if (cmd.getArgs().length > 0) {
                System.err.println("Application does not allow arbitrary arguments: " + Collections.singletonList(cmd.getArgList()));
                System.exit(1);
            }
    ...
    private static final class Converter {
        private final Map<String, List<String>> opts = new LinkedHashMap<>();
        List<String> command;
        ...
        public void add(String option, String suboption) {
            ...
            List<String> params = opts.computeIfAbsent(option, k -> new ArrayList<>());
    ...
    public static void printHelpMessage() {
        ...
        for (Object obj : availableOptions.getOptions()) {
            Option option = (Option) obj;
            String upperCaseName = option.getLongOpt().toUpperCase();
            System.out.printf("-%s%s, --%s%s%n\t\t%s%n%n", option.getOpt(), (option.hasArg()) ? " " + upperCaseName : "",
                    option.getLongOpt(), (option.hasArg()) ? "=" + upperCaseName : "", option.getDescription());
        }
    }

    public static Runnable helpPrinter() {
        return Legacy::printHelpMessage;
    }
