...
public class TimestampSerializer implements TypeSerializer<Date>
{
    ...
    private static final DateTimeFormatter FORMATTER = DateTimeFormatter.ofPattern(DEFAULT_FORMAT).withZone(ZoneId.systemDefault());

    private static final String UTC_FORMAT = dateStringPatterns[40];
    private static final DateTimeFormatter FORMATTER_UTC = DateTimeFormatter.ofPattern(UTC_FORMAT).withZone(ZoneOffset.UTC);

    private static final String TO_JSON_FORMAT = dateStringPatterns[19];
    private static final DateTimeFormatter FORMATTER_TO_JSON = DateTimeFormatter.ofPattern(TO_JSON_FORMAT).withZone(ZoneOffset.UTC);
    ...
    {
        return value == null ? "" : FORMATTER_UTC.format(value.toInstant());
    }
    ...
    public String toCQLLiteral(ByteBuffer buffer)
    {
        return buffer == null || !buffer.hasRemaining()
             ? "null"
             : FORMATTER_UTC.format(deserialize(buffer).toInstant());
    }
}
